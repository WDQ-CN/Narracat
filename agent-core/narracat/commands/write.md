...
3. 合同过后调 `mcp__plugin_narracat_novelmemory__novel_check_prose_hygiene(chapter=chapter_num)`，把返回的 errors（若有）与 fingerprint_findings 全量留给 3d——不派发擦除、不在本步处置。
...
```
Task(chapter-writer): "对第 {chapter_num} 章成稿做一遍冷改打磨，按下面三遍依次处理，改完把整章覆写回正文路径。
正文路径: {manuscript_path}
...
第二遍·缝接缝：通读全文，修指代悬空、没头没尾的反应句、前后语气突变、场景过渡生硬；开头与任务书引用的上一章结尾原文要接得上。
第三遍·补火力：对照任务书点名的心跳节拍，哪里弱了就地重写到「演出来」的强度——只加火力，不改剧情事件、人物名字与去向、不改章末停住的画面。"
```
