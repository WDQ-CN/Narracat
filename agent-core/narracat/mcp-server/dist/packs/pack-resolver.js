...
import { validatePackManifest, DEFAULT_ENABLED_PACK_IDS, } from "./pack-manifest.js";
function defaultBuiltinPacksDir() {
...
export function resetPackResolverCache() { discoveryCache = null; }
function loadPackDir(rootDir, origin, notes) {
    try {
        const raw = JSON.parse(readFileSync(join(rootDir, "pack.json"), "utf8"));
        const { manifest, errors, warnings } = validatePackManifest(raw);
        for (const w of warnings)
            notes.push(`能力包 ${rootDir}：${w}`);
        if (!manifest) {
            notes.push(`能力包 ${rootDir} manifest 非法，已跳过：${errors.join("；")}`);
            return null;
        }
...
function discoverPacks(builtinPacksDir, userPacksDir) {
    const key = `${builtinPacksDir}||${userPacksDir ?? ""}`;
    if (discoveryCache?.key === key)
        return discoveryCache;
    const notes = [];
    const packs = [];
    for (const [dir, origin] of [[builtinPacksDir, "official"], [userPacksDir, "user"]]) {
...
function defaultEnabledEntries() {
...
function dedupeEnabledEntries(entries, notes) {
    const order = [];
    const byId = new Map();
    ...
    for (const id of order) {
        ...
        if (versionKeys.size > 1) {
            notes.push(`启用清单中「${id}」存在冲突版本条目，已全部跳过`);
            continue;
        }
...
function resolveCardPath(cardPath, pack, pluginRoot) {
...
export function resolvePackPools(projectRoot, opts = {}) {
    ...
    const pools = { personas: [], craft: [], structure: [], notes };
    ...
    for (const entry of enabled) {
        ...
        if (!pack) {
            if (!entry.version) {
                notes.push(`已启用的能力包「${entry.id}」缺少版本锁（导入包必须锁版本），已跳过`);
                continue;
            }
            pack = userPacks.find((p) => p.manifest.id === entry.id && p.manifest.version === entry.version);
            if (!pack) {
                notes.push(`已启用的能力包「${entry.id}@${entry.version}」未安装，已跳过`);
                continue;
            }
        }
