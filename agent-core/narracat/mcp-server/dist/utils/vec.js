 ...
 */
export function initVecTable(db, dim = getEmbeddingDim()) {
    try {
        const existing = db
        ...
        if (existing?.sql) {
            ...
            if (existingDim !== null && existingDim !== dim) {
                console.error(`[NovelMemory] memory_vec 维度变更 ${existingDim}→${dim}，重建向量表（存量行将由 backfill 重算）`);
    ...
    `);
        ...
        console.error(`[NovelMemory] 向量表初始化失败，语义检索不可用: ${error instanceof Error ? error.message : String(error)}`);
        vecAvailable = false;
        return false;
    }
}
 ...
 * 插入一条向量记录
 */
export function vecInsert(db, sourceTable, sourceId, novelId, sector, embedding) {
    if (!vecAvailable)
        return;
    db.prepare(`INSERT INTO memory_vec (source_table, source_id, novel_id, sector, embedding)
     VALUES (?, ?, ?, ?, ?)`).run(sourceTable, sourceId, novelId, sector, Buffer.from(embedding.buffer));
}
/**
 * 删除指定 source_id 的向量记录
 */
export function vecDelete(db, sourceTable, sourceId) {
    if (!vecAvailable)
        return;
    db.prepare(`DELETE FROM memory_vec WHERE source_table = ? AND source_id = ?`).run(sourceTable, sourceId);
}
/**
 * 根据 source_id 列表批量删除向量记录
 */
export function vecDeleteBySourceIds(db, sourceTable, sourceIds) {
    if (!vecAvailable || sourceIds.length === 0)
        return;
    const deleteStmt = db.prepare(`DELETE FROM memory_vec WHERE source_table = ? AND source_id = ?`);
 ...
 */
export function vecSearch(db, queryEmbedding, novelId, options) {
    if (!vecAvailable)
        return [];
 ...
 * @returns 本次补算的向量条数
 */
export async function backfillVectors(db, novelId) {
    if (!vecAvailable)
        return { backfilled: 0 };
    ...
    const existing = new Set(db
        ...
        .all(novelId).map((r) => `${r.source_table}:${r.source_id}`));
    const candidates = [];
    for (const r of db
        .prepare("SELECT id, subject, predicate, object FROM facts WHERE novel_id = ?")
        .all(novelId)) {
        if (existing.has(`facts:${r.id}`))
            continue;
        candidates.push({
            sourceTable: "facts",
            sourceId: r.id,
    ...
    for (const r of db
        ...
        .all(novelId)) {
        if (existing.has(`chapter_summaries:${r.id}`))
            continue;
        candidates.push({
    ...
    for (const r of db
        ...
        if (existing.has(`arc_summaries:${sourceId}`))
            continue;
        candidates.push({
    ...
    if (candidates.length === 0)
        return { backfilled: 0 };
    console.error(`[NovelMemory] 向量 backfill：检测到 ${candidates.length} 条历史记忆缺向量，开始补算…`);
    let backfilled = 0;
    for (const c of candidates) {
        const embedding = await embed(c.content);
        if (!embedding) {
            console.error(`[NovelMemory] 向量 backfill 中止：embedding 模型不可用，已补 ${backfilled}/${candidates.length}`);
            break;
        }
