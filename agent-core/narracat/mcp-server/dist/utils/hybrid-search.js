...
import { vecSearch, isVecAvailable } from "./vec.js";
 ...
 */
export async function hybridSearch(db, query, novelId, options) {
    ...
    // 辅助函数：生成去重 key
    const makeKey = (sourceTable, sourceId) => `${sourceTable}:${sourceId}`;
    // FTS 结果评分
 ...
 */
export async function multiQueryHybridSearch(db, queries, novelId, options) {
    const limit = options?.limit ?? 10;
