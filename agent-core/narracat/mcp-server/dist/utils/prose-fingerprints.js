 ...
 */
export function scanProseFingerprints(text) {
    ...
    for (const category of PROSE_FINGERPRINT_LEXICON) {
        ...
        if (category.mode === "term") {
            ...
            for (const m of matches) {
                ...
                entry.count += 1;
                if (entry.positions.length < FINGERPRINT_HIT_POSITIONS_LIMIT)
                    entry.positions.push(m.index ?? 0);
                byTerm.set(m[0], entry);
            }
            for (const [term, entry] of byTerm)
                hits.push({ term, count: entry.count, positions: entry.positions });
        }
        if (total > 0) {
            findings.push({
