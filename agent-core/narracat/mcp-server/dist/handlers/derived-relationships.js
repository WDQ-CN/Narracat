...
export function computeDerivedRelationships(args) {
    ...
    const nameOnEdge = (e, uid) => e.aUid === uid ? e.aName : e.bName;
    ...
    for (let i = 0; i < sorted.length; i++) {
        for (let j = i + 1; j < sorted.length; j++) {
            ...
            if (bPeers?.has(c.uid) || directPairKeys?.has(pairKey))
                continue;
            if (!bPeers || !cPeers)
                continue;
            ...
            const via = [...bPeers.keys()]
                ...
                .sort((l, r) => (adjacency.get(l)?.size ?? 0) - (adjacency.get(r)?.size ?? 0) ||
                l.localeCompare(r))[0];
            if (!via)
                continue;
            ...
            const line = `${b.name} 与 ${c.name} 或有关联（推断）：` +
    ...
    candidates.sort((l, r) => l.viaDegree - r.viaDegree || l.pairKey.localeCompare(r.pairKey));
 ...
 */
export function loadValidRelationshipEdges(ctx, asOfChapter) {
    const rows = ctx.db
        .prepare(`SELECT subject, subject_character_uid AS a_uid, subject_character_b_uid AS b_uid,
        ...
        .all(ctx.novelId, asOfChapter, asOfChapter);
    const seen = new Set();
    const edges = [];
...
export function loadFactCountByUid(ctx, asOfChapter) {
    const rows = ctx.db
        .prepare(`SELECT subject_character_uid AS a_uid, subject_character_b_uid AS b_uid
         FROM facts
    ...
    for (const row of rows) {
        if (row.a_uid)
            counts.set(row.a_uid, (counts.get(row.a_uid) ?? 0) + 1);
        if (row.b_uid)
            counts.set(row.b_uid, (counts.get(row.b_uid) ?? 0) + 1);
    }
    return counts;
}
