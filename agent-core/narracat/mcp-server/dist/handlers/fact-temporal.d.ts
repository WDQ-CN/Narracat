...
export declare const FACT_VALID_AT_SQL = "COALESCE(event_chapter, from_chapter) <= ? AND (invalidated_at_chapter IS NULL OR invalidated_at_chapter > ?)";
/**
 ...
 */
export declare const FACT_LATEST_ORDER_SQL = "COALESCE(event_chapter, from_chapter) DESC, CASE WHEN source = 'authored' THEN 1 ELSE 0 END DESC, from_chapter DESC, created_at DESC, rowid DESC";
...
export declare function isFactValidAt(row: TemporalFactRow, at: number): boolean;
/**
 ...
 */
export declare const SECRET_FILTER_SQL = "NOT (predicate = 'secret' AND COALESCE(secret_known, 0) = 0)";
export interface SecretGateRow {
    predicate: string;
 ...
 */
export declare function isSecretHiddenForChat(row: SecretGateRow, secretFilterOn: boolean): boolean;
/**
 ...
 */
export declare function revalidateVictimsOf(db: Database.Database, novelId: string, revokedId: string): number;
