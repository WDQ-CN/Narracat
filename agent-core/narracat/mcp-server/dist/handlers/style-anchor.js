...
export function listStyleAnchorRows(ctx) {
    return ctx.db
        .prepare(`SELECT anchor_id, chapter, excerpt, created_at FROM style_anchors
...
export async function novelSubmitStyleAnchor(args, ctx) {
    ...
    if (chars < MIN_EXCERPT_CHARS || chars > MAX_EXCERPT_CHARS) {
        return singleError("excerpt", `${MIN_EXCERPT_CHARS}-${MAX_EXCERPT_CHARS} 字`, `${chars} 字`, `样章取 ${MIN_EXCERPT_CHARS}-${MAX_EXCERPT_CHARS} 字：太短读不出语感，太长挤占写手的上下文`);
    }
    const existing = listStyleAnchorRows(ctx);
    if (existing.length >= MAX_ANCHORS) {
        return singleError("excerpt", `最多 ${MAX_ANCHORS} 段样章`, `已有 ${existing.length} 段`, `已有 ${MAX_ANCHORS} 段样章，先删一段再标新的`);
    }
    ...
    if (!isChapterCommitted(ctx, chapter)) {
        return singleError("chapter", "该章已完成并收尾入库", `第 ${chapter} 章尚未收尾`, "这一章还没写完，写完并收尾后再标样章");
    }
    const content = await readFile(manuscriptPath, "utf-8");
    if (!stripWhitespace(content).includes(stripWhitespace(excerpt))) {
        return singleError("excerpt", "该段落出现在本章正文中", "正文中未找到这段文字", "样章必须是本章正文里的原话；正文若已改动，请重新划选当前正文里的段落");
    }
    ...
    ctx.db
        .prepare(`INSERT INTO style_anchors (novel_id, anchor_id, chapter, excerpt) VALUES (?, ?, ?, ?)`)
        .run(ctx.novelId, anchorId, chapter, excerpt);
    return { ok: true, anchor_id: anchorId, total: existing.length + 1 };
}
export async function novelListStyleAnchors(_args, ctx) {
