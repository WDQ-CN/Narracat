 ...
 */
export async function loadAliasMap(projectRoot) {
    const map = new Map();
    ...
    for (const file of files) {
        ...
        if (idMatch) {
            try {
                const parsed = JSON.parse(idMatch[1]);
