...
const VALID_SOURCES = ["plan", "write", "manual"];
const VALID_STATUSES = ["candidate", "promoted"];
const VALID_IMPORTANCE = ["minor", "major"];
/**
 ...
 */
export async function novelRegisterCandidateCharacter(args, ctx) {
    const name = args["name"];
    if (typeof name !== "string" || !name.trim()) {
        return singleError("name", "非空字符串", name === undefined ? "missing" : String(name), "候选角色必须有显示名（name）");
    }
    ...
    if (uid === undefined || uid === null || uid === "") {
        ...
        return singleError("character_uid", "lowercase UUID v4，或省略由工具铸造", String(uid), "character_uid 须为经 novel_mint_character_uid 铸造的 lowercase UUID v4；新候选可省略此字段由工具自动铸造");
    }
    const source = args["source"];
    if (source !== undefined && !VALID_SOURCES.includes(source)) {
        return singleError("source", `${VALID_SOURCES.join(" / ")}`, String(source), "source 只接受 plan / write / manual");
    }
    const status = args["status"];
    if (status !== undefined && !VALID_STATUSES.includes(status)) {
        return singleError("status", `${VALID_STATUSES.join(" / ")}`, String(status), "status 只接受 candidate / promoted");
    }
    const importanceArg = args["importance"];
    if (importanceArg !== undefined && !VALID_IMPORTANCE.includes(importanceArg)) {
        return singleError("importance", `${VALID_IMPORTANCE.join(" / ")}`, String(importanceArg), "importance 只接受 minor（次要，进池静默不提醒）/ major（重要，写完正文提醒建档）；省略 = minor");
    }
    // 省略时绑 null → 新插入经 COALESCE 落默认 'minor'、冲突更新保留既有（promote 回写不清掉重要度）。
    const importance = importanceArg ?? null;
    const proposedChapterArg = args["proposed_chapter"];
    let proposedChapter = null;
    if (proposedChapterArg !== undefined && proposedChapterArg !== null) {
        if (typeof proposedChapterArg !== "number" ||
            !Number.isInteger(proposedChapterArg) ||
            proposedChapterArg < 1) {
            return singleError("proposed_chapter", "integer ≥ 1，或省略", String(proposedChapterArg), "proposed_chapter 是该角色计划首次出场/被提及的章号，须为正整数或省略");
        }
    ...
    let initialRelationshipsJson = null;
    if (initialRelationshipsArg !== undefined && initialRelationshipsArg !== null) {
        if (!Array.isArray(initialRelationshipsArg)) {
            return singleError("initial_relationships", "数组，或省略", String(initialRelationshipsArg), "initial_relationships 是 [{other_character_uid, state}] 数组");
        }
        for (const rel of initialRelationshipsArg) {
            const other = rel?.["other_character_uid"];
            const state = rel?.["state"];
            if (typeof other !== "string" || !other.trim() || typeof state !== "string" || !state.trim()) {
                return singleError("initial_relationships[]", "每项含非空 other_character_uid + state", JSON.stringify(rel), "每个关系项须有 other_character_uid（已建档角色 uid）与 state（关系状态一句话）");
            }
        }
        initialRelationshipsJson = JSON.stringify(initialRelationshipsArg.map((rel) => ({
            other_character_uid: rel["other_character_uid"].trim(),
            state: rel["state"].trim(),
        })));
    }
    const note = typeof args["note"] === "string" && args["note"].trim()
        ? args["note"].trim()
        : null;
    const resolvedSource = source ?? "write";
    const resolvedStatus = status ?? "candidate";
    const characterUid = uid;
    const characterName = name.trim();
    ctx.db
        .prepare(`INSERT INTO candidate_characters
       ...
       ON CONFLICT(novel_id, character_uid) DO UPDATE SET
        ...
        .run(ctx.novelId, characterUid, characterName, note, proposedChapter, initialRelationshipsJson, importance, resolvedSource, resolvedStatus, initialRelationshipsJson, importance);
    return {
        ok: true,
        character_uid: characterUid,
        name: characterName,
        status: resolvedStatus,
        message: resolvedStatus === "promoted"
 ...
 */
export async function novelListCandidateCharacters(args, ctx) {
    const statusArg = args["status"];
    let where = "novel_id = ? AND status = 'candidate'";
    const params = [ctx.novelId];
    if (statusArg === "promoted") {
        ...
        return singleError("status", "candidate / promoted / all（省略 = candidate）", String(statusArg), "status 只接受 candidate / promoted / all");
    }
    // 重要度过滤（ADR-0023）：省略 = 全部（App 目录用）；major = 写完正文只提醒的重要候选。
    const importanceArg = args["importance"];
    if (importanceArg !== undefined && !VALID_IMPORTANCE.includes(importanceArg)) {
        return singleError("importance", `${VALID_IMPORTANCE.join(" / ")}（省略 = 全部）`, String(importanceArg), "importance 过滤只接受 minor / major");
    }
    ...
    const rows = ctx.db
        .prepare(`SELECT character_uid, name, note, proposed_chapter, initial_relationships, importance, source, status
...
function parseInitialRelationships(raw) {
    ...
    try {
        ...
        if (!Array.isArray(parsed))
            return [];
        return parsed.filter((rel) => Boolean(rel) &&
            typeof rel === "object" &&
            typeof rel.other_character_uid === "string" &&
            typeof rel.state === "string");
    }
    catch {
        return [];
    }
