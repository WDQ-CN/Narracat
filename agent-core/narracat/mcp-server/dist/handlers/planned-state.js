 ...
 */
export function mirrorChapterPlannedState(db, novelId, chapter, entries, dedupe) {
    db.prepare(`DELETE FROM planned_state_changes WHERE novel_id = ? AND chapter = ? AND status = 'planned'`).run(novelId, chapter);
    const findSameKeyAnyStatus = db.prepare(`SELECT id FROM planned_state_changes
     WHERE novel_id = ? AND chapter = ? AND character_uid = ? AND dimension = ? AND operation = ? AND value = ?`);
    const findSameKeyPlannedOnly = db.prepare(`SELECT id FROM planned_state_changes
     WHERE novel_id = ? AND chapter = ? AND character_uid = ? AND dimension = ? AND operation = ? AND value = ?
       AND status = 'planned'`);
    const findSameKey = dedupe === "any-status" ? findSameKeyAnyStatus : findSameKeyPlannedOnly;
    const insert = db.prepare(`INSERT INTO planned_state_changes
       (id, novel_id, chapter, character_uid, character_name, dimension, operation, value, reason)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
    for (const e of entries) {
        if (findSameKey.get(novelId, chapter, e.character_uid, e.dimension, e.operation, e.value))
            continue;
...
export async function locateChapterOutlineFile(projectRoot, chapter) {
    ...
    for (const ent of entries) {
        if (!ent.isDirectory() || !/^vol-\d+$/.test(ent.name))
            continue;
...
export function buildChapterRenderContext(ctx) {
    const storylineRows = ctx.db.prepare(`SELECT id, name FROM storylines WHERE novel_id = ?`).all(ctx.novelId);
    const registryRows = ctx.db.prepare(`SELECT id, description FROM foreshadowing_registry WHERE novel_id = ?`).all(ctx.novelId);
    return {
 ...
 */
async function replaceOutlineFilesThenDb(paths, next, dbWrite) {
    ...
    try {
        ...
        if (oldMd === null)
            await unlink(paths.mdPath).catch(() => { });
        else
            await writeFile(paths.mdPath, oldMd, "utf-8").catch(() => { });
        throw error;
    }
}
function describeItem(row, displayName) {
    const op = row.operation === "remove" ? "失去" : row.operation === "add" ? "获得" : "变为";
    return `${row.character_name} 的${displayName}${op}「${row.value}」`;
}
export async function novelCheckStateDelivery(args, ctx) {
    ...
    const rows = ctx.db
        .prepare(`SELECT * FROM planned_state_changes
    ...
    const findDeliveredFact = ctx.db.prepare(`SELECT id FROM facts
    ...
    const findRemovedFact = ctx.db.prepare(`SELECT id FROM facts
     WHERE novel_id = ? AND subject_character_uid = ? AND subject_character_b_uid IS NULL
       AND predicate = ? AND object = ? AND invalidated_at_chapter = ?
       AND COALESCE(event_chapter, from_chapter) < invalidated_at_chapter
     LIMIT 1`);
    const markDelivered = ctx.db.prepare(`UPDATE planned_state_changes SET status = 'delivered', updated_at = datetime('now') WHERE id = ?`);
    const delivered = [];
    const undelivered = [];
    const tx = ctx.db.transaction(() => {
        for (const row of rows) {
            ...
            const hit = row.operation === "remove"
    ...
    return {
        ...
        delivered,
        undelivered,
        message: undelivered.length === 0
            ? `本章 ${rows.length} 项计划状态变更全部兑现`
            : `本章计划状态变更 ${rows.length} 项：${delivered.length} 项已兑现，${undelivered.length} 项未在正文记忆中找到`,
    };
}
const RESOLVE_ACTIONS = new Set(["defer", "cancel", "acknowledge", "mark_delivered"]);
export async function novelResolvePlannedState(args, ctx) {
    ...
    if (typeof action !== "string" || !RESOLVE_ACTIONS.has(action)) {
    ...
    if (action === "defer") {
        ...
        let targetJson;
        try {
            targetJson = JSON.parse(await readFile(located.jsonPath, "utf-8"));
        }
        catch {
        ...
        if (!Array.isArray(targetJson.beats)) {
        ...
        const resolveTargetOp = (sc) => (typeof sc["operation"] === "string" ? sc["operation"] : undefined) ??
            (dimCardinality.get(sc["dimension"]) === "one" ? "set" : "add");
        const sameKey = targetList.some((sc) => {
            const character = (sc["character"] ?? {});
            return (character["character_uid"] === row.character_uid &&
        ...
        const dbWrite = () => {
            const tx = ctx.db.transaction(() => {
                ctx.db
                    .prepare(`UPDATE planned_state_changes
             SET status = 'deferred', deferred_to_chapter = ?, updated_at = datetime('now') WHERE id = ?`)
                ...
                const dup = ctx.db
                    .prepare(`SELECT id FROM planned_state_changes
             WHERE novel_id = ? AND chapter = ? AND character_uid = ? AND dimension = ? AND operation = ? AND value = ?
               AND status = 'planned'`)
                    .get(ctx.novelId, toChapter, row.character_uid, row.dimension, row.operation, row.value);
                if (!dup) {
                    ctx.db
                        .prepare(`INSERT INTO planned_state_changes
                 (id, novel_id, chapter, character_uid, character_name, dimension, operation, value, reason)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                        .run(newId, ctx.novelId, toChapter, row.character_uid, row.character_name, row.dimension, row.operation, row.value, row.reason);
                }
                return dup?.id;
            });
            return tx();
        ...
        try {
            if (sameKey) {
                existingId = dbWrite(); // json 无需改动，只落 DB
            }
            else {
                const nextJson = { ...targetJson, state_changes: targetList };
                const mdText = `${renderChapterOutlineMarkdown(nextJson, buildChapterRenderContext(ctx))}\n`;
...
function normalizeStateChangeEntry(raw, index) {
    const r = (raw ?? {});
    ...
    if (!uid || !name || !dimension || !value)
        return { error: `第 ${index + 1} 条缺少角色/维度/值` };
    if (value.length > 60)
        return { error: `第 ${index + 1} 条值超 60 字` };
    if (reason.length > 100)
        return { error: `第 ${index + 1} 条缘由超 100 字` };
    if (operation !== undefined && operation !== "set" && operation !== "add" && operation !== "remove") {
        return { error: `第 ${index + 1} 条 operation 非法` };
    }
    const entry = { character: { character_uid: uid, name }, dimension, value };
 ...
 */
export async function novelUpdateChapterStateChanges(args, ctx) {
    const payload = (args["payload"] ?? {});
    ...
    const vocab = loadStateVocabulary(ctx.projectRoot);
    const gateErrors = checkStateChanges([{ chapter, state_changes: entries }], vocab);
    if (gateErrors.length > 0)
        return errorResponse(gateErrors);
    ...
    let json;
    try {
        json = JSON.parse(await readFile(located.jsonPath, "utf-8"));
    }
    catch {
    ...
    const mdText = `${renderChapterOutlineMarkdown(nextJson, renderCtx)}\n`;
    const dimCardinality = new Map((vocab?.dimensions ?? []).map((d) => [d.key, d.cardinality]));
