...
import { validateStateVocabulary, validateCharacterEntity, validateAuthoredState, checkStateVocabularySemantics, } from "./validators.js";
import { loadStateVocabulary, STATE_VOCABULARY_RELPATH, attributeFact } from "./state-dimensions.js";
...
export async function novelSubmitStateVocabulary(args, ctx) {
 ...
 */
function buildIdentityLine(md, uid, name) {
    let extra = {};
...
function syncIdentityIntoMarkdown(md, uid, name, aliases) {
 ...
 */
async function readIdentityUidFromMarkdown(mdPath) {
    let md;
    try {
 ...
 */
async function migrateRenamedEntityFiles(dir, uid, newName) {
    const warnings = [];
    let files;
    try {
    ...
    for (const file of files) {
        ...
        if (oldEntity.character_uid !== uid)
            continue;
        ...
        if (oldMdExists && newMdExists) {
            warnings.push(`角色改名冲突：检测到 character_uid ${uid} 从「${oldName}」改名为「${newName}」，但两者的档案 md 文件同时存在，未自动处理，请人工合并（保留一份、把另一名加入 aliases）`);
            continue;
        }
        if (oldMdExists) {
        ...
        warnings.push(`角色改名：${oldName} → ${newName}，档案文件已随迁；若正文仍用旧名称呼，请把旧名加入 aliases`);
    }
    return warnings;
}
export async function novelSubmitCharacterEntity(args, ctx) {
    const check = validateCharacterEntity(args["payload"]);
    if (!check.valid)
        return errorResponse(check.errors);
    const payload = args["payload"];
    const vocab = loadStateVocabulary(ctx.projectRoot);
    const stateErrors = checkInitialStates(payload.initial_states, vocab);
    if (stateErrors.length > 0)
        return errorResponse(stateErrors);
    ...
    try {
        const existingEntity = JSON.parse(await readFile(targetJsonPath, "utf-8"));
        if (typeof existingEntity.character_uid === "string")
            existingUid = existingEntity.character_uid;
    }
    catch {
        existingUid = null;
    }
    ...
    const tx = ctx.db.transaction(() => {
        const insert = ctx.db.prepare(`INSERT INTO facts (id, novel_id, subject, subject_character_uid, predicate, object, sector, from_chapter, event_chapter, source)
       VALUES (?, ?, ?, ?, ?, ?, 'semantic', ?, ?, 'authored')`);
        for (const s of payload.initial_states ?? []) {
            ...
            const dup = ctx.db
                .prepare(`SELECT id FROM facts WHERE novel_id=? AND subject_character_uid=? AND predicate=? AND object=?
            ...
            if (dim.cardinality === "one" && dim.value_type === "enum" && dim.values) {
                ...
                ctx.db
                    .prepare(`UPDATE facts SET invalidated_at_chapter=?, invalidated_by=?, updated_at=datetime('now')
        ...
        ctx.db
            .prepare(`UPDATE candidate_characters SET status='promoted', updated_at=datetime('now')
...
function invalidateFactById(ctx, factId, atChapter, replacedBy) {
    ctx.db
        .prepare(`UPDATE facts SET invalidated_at_chapter=?, invalidated_by=?, updated_at=datetime('now')
...
function latestSummarizedChapter(ctx) {
    const row = ctx.db
        .prepare(`SELECT MAX(chapter) AS c FROM chapter_summaries WHERE novel_id=?`)
        .get(ctx.novelId);
    return row.c ?? 0;
}
export async function novelSubmitAuthoredState(args, ctx) {
    ...
    for (const [, resolved] of aliasMap) {
        if (resolved.uid === uid) {
            subject = resolved.canonical;
            break;
        }
    }
    if (!subject) {
        const row = ctx.db
            .prepare(`SELECT subject FROM facts WHERE novel_id=? AND subject_character_uid=? ORDER BY created_at DESC LIMIT 1`)
            .get(ctx.novelId, uid);
    ...
    if (p.action === "correct" || p.action === "retract" || p.action === "endorse" || p.action === "mark_secret_known") {
        ...
        const target = ctx.db
            .prepare(`SELECT id, subject, predicate, object, from_chapter, event_chapter, invalidated_at_chapter, invalidated_by, source, secret_known
        ...
        if (!target) {
            const isRelationship = ctx.db
                .prepare(`SELECT 1 FROM facts WHERE id=? AND novel_id=? AND subject_character_uid=? AND subject_character_b_uid IS NOT NULL`)
                .get(p.target_fact_id, ctx.novelId, uid);
            if (isRelationship) {
                return singleError("target_fact_id", "非关系类事实记录", p.target_fact_id ?? "", "关系事实不支持经本工具修订，关系修订走后续关系编辑入口");
            }
        ...
        if (p.new_value !== undefined && vocab) {
            ...
            if (enumDims.length > 0 && !hasFreeDim && !matchesSomeEnum) {
                ...
                return singleError("new_value", `该谓词下词表值域内的值（${allowedValues}）`, p.new_value, "扩词表加值（重交 novel_submit_state_vocabulary）或纠正为值域内的值");
            }
        }
        ...
        const naturallySupersededAt = target.invalidated_at_chapter !== null && target.invalidated_at_chapter > targetEvent
        ...
        const tx = ctx.db.transaction(() => {
            ctx.db
                .prepare(`INSERT INTO facts (id, novel_id, subject, subject_character_uid, predicate, object, sector, from_chapter, event_chapter, source, secret_known)
           VALUES (?, ?, ?, ?, ?, ?, 'semantic', ?, ?, 'authored', ?)`)
            ...
            if (newEvent === targetEvent) {
                ctx.db
                    .prepare(`UPDATE facts SET invalidated_by=?, updated_at=datetime('now') WHERE novel_id=? AND invalidated_by=?`)
            ...
            if (naturallySupersededAt !== null) {
                ctx.db
                    .prepare(`UPDATE facts SET invalidated_at_chapter=?, invalidated_by=?, updated_at=datetime('now') WHERE id=? AND novel_id=?`)
    ...
    if (p.action === "backfill" && (p.operation === "set" || p.operation === "remove")) {
        return singleError("operation", "backfill 不接受显式 operation：单值维度自动 set、集合维度自动 add；历史失效走时间线作废/纠错", p.operation, "历史失效走时间线作废（retract）或纠错（correct），backfill 不接受 set/remove");
    }
    ...
    const validRows = ctx.db
        .prepare(`SELECT id, subject, predicate, object, from_chapter, event_chapter, invalidated_at_chapter, source
       ...
       ORDER BY ${FACT_LATEST_ORDER_SQL}`)
        .all(ctx.novelId, uid, dim.predicate);
    ...
    if (p.action === "set_current" && p.expected_current_value !== undefined) {
        ...
        const cardRows = ctx.db
            .prepare(`SELECT object FROM facts WHERE novel_id=? AND subject_character_uid=? AND subject_character_b_uid IS NULL
           AND predicate=? AND ${FACT_VALID_AT_SQL}
         ORDER BY ${FACT_LATEST_ORDER_SQL}`)
            .all(ctx.novelId, uid, dim.predicate, cardAsOf, cardAsOf);
