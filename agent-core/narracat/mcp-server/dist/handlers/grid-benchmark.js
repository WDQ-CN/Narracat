 ...
 */
export function resolveDriveBucket(genre) {
    const g = (genre ?? "").trim();
    if (g) {
...
// ============================================================
export const GRID_BENCHMARK_TABLE = {
 ...
 */
export const ARC_VELOCITY_TARGET_TABLE = {
 ...
 * 同款读盘容错，不阻断。返回按章号升序。
 */
export async function collectSubmittedChapters(ctx) {
    const arcRows = ctx.db
        .prepare(`SELECT arc_id, volume_no, chapter_start, chapter_end FROM arc_meta WHERE novel_id = ?`)
        .all(ctx.novelId);
    const out = [];
...
export function computeHookNoneRate(chapters) {
...
export function computePayoffInterval(chapters) {
    ...
    for (const c of sorted) {
        if (c.payoff_beat == null)
            continue;
        if (lastChapter !== null)
            gaps.push(c.chapter - lastChapter);
 ...
 */
function intervalBandLabel(value, q) {
    if (value > q.p90)
        return "高于同类章间隔P90（同类头部书少见）";
    if (value > q.p75)
        return "高于同类章间隔P75";
    if (value > q.median)
        return "略高于同类章间隔中位";
 ...
 */
export function rankAmongBooks(value, sortedRatesAsc) {
...
export async function buildGridBenchmark(ctx) {
    ...
    if (interval) {
        ...
        noteParts.push(`爽点间隔${label}（同类头部章间隔中位约${table.payoff_interval_chapters.median}章，本书约${interval.median}章）`);
    }
...
export async function novelGetGridBenchmark(_args, ctx) {
 ...
 */
export async function buildBenchmarkNote(ctx) {
    const result = await buildGridBenchmark(ctx);
    if (result.sample_size < GRID_BENCHMARK_THRESHOLDS.min_sample_chapters)
        return null;
    return result.note;
}
/** 取本书 arc 层定速靶（前馈用）。brief 是随包投递给架构师的一句人话靶——含数字，因为它是数据不是 agent prompt。 */
export function buildArcVelocityTarget(ctx) {
    const resolution = resolveDriveBucket(ctx.genre);
    ...
    const brief = `同类头部节奏靶${approx}：主线别连续休眠超 ${maxDormancyRunDisplay} 章（一段没有具体对抗、没往前推的戏），` +
...
export async function novelGetArcVelocityTarget(_args, ctx) {
