...
function resolvePackIndexPath() {
    const dir = path.dirname(fileURLToPath(import.meta.url));
    // dist/ → ../../skills/novel-web-craft/references/pack-index.json
    return path.resolve(dir, "../../skills/novel-web-craft/references/pack-index.json");
}
/**
 ...
 */
function expandReferencePath(rawPath) {
    return rawPath.replace(/\$\{CLAUDE_PLUGIN_ROOT\}|\$CLAUDE_PLUGIN_ROOT\b/g, resolvePluginRoot());
}
export function loadPackIndex() {
    if (packIndexCache !== null)
...
/** 测试用：清空模块缓存 */
export function _resetPackIndexCache() {
    packIndexCache = null;
}
/**
 ...
 */
export function defaultCraftPool() {
...
export function selectCraftPacks(chapterOutline, limit = 3, pool) {
    if (!chapterOutline)
        return [];
    ...
    for (const entry of packs) {
        if (entry.exclusions.some((x) => x && chapterOutline.includes(x)))
            continue;
        const reasons = [];
    ...
    const tierOf = (origin) => (origin === "user" ? 0 : 1);
    scored.sort((a, b) => {
        ...
        const rankDiff = (ORIGIN_RANK[a.entry.origin] ?? 9) - (ORIGIN_RANK[b.entry.origin] ?? 9);
        if (rankDiff !== 0)
            return rankDiff;
