 ...
 */
function loadEntriesFromDir(dir) {
    ...
    for (const file of files) {
        try {
            ...
            console.error(`[NovelMemory] Failed to load corpus file ${file}:`, err);
        }
    }
    console.error(`[NovelMemory] Loaded ${entries.length} style reference entries from ${files.length} files`);
    entriesByDir.set(dir, entries);
    return entries;
}
/** 从记录 id（WK-031-001）取 work_id（WK-031），供「同书不重复」去重。导出供单测。 */
export function workIdOf(id) {
    return id.replace(/-\d+$/, "");
}
...
export function isPayoffCoolingAnnotation(annotation) {
...
export function hasVividTechnique(technique) {
...
export function annotationCarriesRestraintVocab(annotation) {
 ...
 */
export function detectChapterEmotions(text) {
    if (!text)
        return [];
    const scored = [];
 ...
 */
export function selectStyleExamplesFrom(entries, chapter, limit = 3, chapterEmotions = []) {
    const filtered = entries.filter((e) => !isPayoffCoolingAnnotation(e.annotation));
    ...
    const vividClean = vivid.filter((e) => !annotationCarriesRestraintVocab(e.annotation));
    const vividRestraint = vivid.filter((e) => annotationCarriesRestraintVocab(e.annotation));
    const rest = sorted.filter((e) => !hasVividTechnique(e.technique));
    const emoSet = new Set(chapterEmotions);
    const matchesEmotion = (e) => emoSet.size > 0 && e.emotion.some((x) => emoSet.has(x));
    const picked = [];
    const usedWorkIds = new Set();
    // 档内取段：按章号确定性轮换 + 优先不同作品（这就是「同情绪多条」时的取舍规则）
    const rotatePick = (group) => {
        if (group.length === 0)
            return;
        ...
        for (let i = 0; i < group.length && picked.length < count; i += 1) {
            ...
            if (picked.includes(entry) || usedWorkIds.has(workIdOf(entry.id)))
                continue;
            picked.push(entry);
 ...
 */
export function queryStyleReferenceFrom(entries, query) {
    ...
    const scored = entries
        ...
        let score = 0;
        // 手法匹配（至少匹配一个手法，否则排除）
        const matchedTechniques = entry.technique.filter((t) => techniqueSet.has(t));
        if (matchedTechniques.length === 0)
            return null;
        score += matchedTechniques.length * 10;
        // 情感匹配（可选加分）
        if (emotionSet) {
            const matchedEmotions = entry.emotion.filter((e) => emotionSet.has(e));
 ...
 */
export function resolveCorpusSource(env = process.env) {
...
export function __resetCorpusCachesForTest() {
    remoteCache.clear();
    entriesByDir.clear();
}
function isCorpusApiBadRequest(v) {
    return typeof v === "object" && v !== null && v.badRequest === true;
}
async function postCorpusApi(path, body, source) {
 ...
 */
export async function selectStyleExamples(chapter, limit = 3, chapterEmotions = []) {
    const source = resolveCorpusSource();
    if (source.mode === "disabled")
        return [];
    if (source.mode === "local") {
        return selectStyleExamplesFrom(loadEntriesFromDir(source.dir), chapter, limit, chapterEmotions);
    }
    const key = `select:${chapter}:${limit}:${[...chapterEmotions].sort().join(",")}`;
    const cached = remoteCache.get(key);
    if (cached)
        return cached;
    const apiResp = await postCorpusApi("/v1/select-examples", { chapter, limit, emotions: chapterEmotions }, source);
    if (isCorpusApiBadRequest(apiResp))
        return []; // 入参越界：按空处理，不缓存
    const resp = apiResp;
    if (!resp?.ok || !Array.isArray(resp.examples))
        return []; // 失败不缓存
 ...
 */
export async function queryStyleReference(query) {
    const source = resolveCorpusSource();
    if (source.mode === "disabled") {
