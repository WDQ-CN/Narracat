...
export function initSchema(db) {
    ...
    if (currentVersion < SCHEMA_VERSION) {
        for (let target = currentVersion + 1; target <= SCHEMA_VERSION; target += 1) {
            ...
            if (!migrate) {
                throw new Error(`memory.db schema 无法从 ${currentVersion} 升到 ${SCHEMA_VERSION}：缺 v${target} 的加列迁移` +
    ...
    throw new Error(`memory.db schema 版本过新（当前 ${currentVersion}，本版本支持 ${SCHEMA_VERSION}），不支持降级。`);
}
/**
 ...
 */
export function recordNovelId(db, novelId) {
    if (!novelId) {
    ...
    const row = db.prepare("SELECT value FROM meta WHERE key = 'novel_id'").get();
    if (row?.value === novelId)
        return;
    if (row) {
        console.error(`[NovelMemory] meta.novel_id（${row.value}）与 config.yaml（${novelId}）不一致，以 config 为准覆盖。`);
    }
    db.prepare("INSERT INTO meta (key, value) VALUES ('novel_id', ?) " +
        "ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(novelId);
}
