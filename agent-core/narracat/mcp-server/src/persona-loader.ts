...
function resolvePersonasDir(): string {
  ...
  return path.resolve(
    __dirname,
    "../../skills/novel-web-craft/references/personas",
  );
}

let personaIndex: PersonaIndexEntry[] | null = null;

function loadPersonaIndex(): PersonaIndexEntry[] {
  if (personaIndex !== null) return personaIndex;
  ...
  try {
    ...
    personaIndex = (data.personas ?? []).filter(
      (p) => p.id && p.file && Array.isArray(p.keywords),
    );
  } catch (err) {
 ...
 */
export function defaultPersonaPool(): PersonaPoolEntry[] {
...
export function selectPersona(
  ...
  const freeText = [
    ...
    .filter(Boolean)
    .join(" ");
  if (!archetype && !freeText) return null;

  const entries = pool ?? defaultPersonaPool();
  let bestScore = 0;
  let top: PersonaPoolEntry[] = [];
  const userHits: PersonaPoolEntry[] = [];
  let userBestScore = 0;
  let userTop: PersonaPoolEntry[] = [];
  for (const entry of entries) {
    let score = 0;
    for (const kw of entry.keywords) {
      if (archetype.includes(kw)) score += 2;
      if (freeText.includes(kw)) score += 1;
    }
    if (score > bestScore) {
      bestScore = score;
      top = [entry];
  ...
  let winners: PersonaPoolEntry[];
  if (userHits.length > 0) {
    winners = userTop.length === 1 ? userTop : [];
  } else {
    if (top.length === 0 || bestScore === 0) return null;
  ...
  if (!best.path) return { id: best.id, name: best.name, body: "" };
