...
export type ForeshadowingLifecycleIssueType =
  | "duplicate_plant_action"
  | "registry_planted_chapter_conflict";
...
interface RegistryPlantRow {
  id: string;
  planted_chapter: number | null;
}

interface ActionPlantRow {
  foreshadowing_id: string;
  chapter: number;
  action: string;
  status: string;
...
export function auditForeshadowingLifecycle(
  db: Database.Database,
  novelId: string,
  options: { foreshadowingIds?: string[] } = {},
  ...
  const registryPlants = (
    db
      .prepare(
        `SELECT id, planted_chapter
         FROM foreshadowing_registry
         WHERE novel_id = ?
         ORDER BY id`,
      )
      .all(novelId) as RegistryPlantRow[]
  ).filter((row) => shouldInclude(row.id) && row.planted_chapter !== null);

  const actionPlants = (
    db
      .prepare(
        `SELECT foreshadowing_id, chapter, action, status
         FROM foreshadowing_actions_log
         WHERE novel_id = ? AND action = 'plant'
         ORDER BY foreshadowing_id, chapter, status`,
      )
      .all(novelId) as ActionPlantRow[]
  ).filter((row) => shouldInclude(row.foreshadowing_id));

  const ids = new Set<string>();
  for (const row of registryPlants) ids.add(row.id);
  for (const row of actionPlants) ids.add(row.foreshadowing_id);

  const findings: ForeshadowingLifecycleFinding[] = [];
  for (const id of Array.from(ids).sort()) {
    ...
    if (actionChapters.length > 1) {
      findings.push({
        ...
        suggestion:
          "保留最早章节的 plant action；后续章节应按 develop 或 reveal 重新记录，若无法判断实际动作，先删除/忽略后续 plant action 并人工复核章节正文。",
      });
    }

    const realizedChapters = uniqueSortedNumbers(
      actionsForId.filter((row) => row.status === "realized").map((row) => row.chapter),
    );
    const earliestRealizedPlant = realizedChapters[0];
    if (
      registryForId?.planted_chapter !== null &&
      ...
      findings.push({
        ...
        suggestion:
          "registry.planted_chapter 应指向最早真实 plant 章节；若后续 register 覆盖了 planted_chapter，手动恢复为最早章节并保留后续章节为 develop/reveal。",
      });
    }
  }
  ...
  return {
    ...
    summary: {
      ...
      duplicate_plant_actions: findings.filter(
        (finding) => finding.issue_type === "duplicate_plant_action",
      ).length,
      registry_planted_chapter_conflicts: findings.filter(
        (finding) => finding.issue_type === "registry_planted_chapter_conflict",
      ).length,
    },
  };
}

export function formatForeshadowingLifecycleAuditReport(
  report: ForeshadowingLifecycleAuditReport,
): string {
  const lines = [
  ...
  if (report.findings.length === 0) {
    ...
    for (const finding of report.findings) {
      lines.push(
        `- ${finding.foreshadowing_id} [${finding.issue_type}] chapters=${finding.chapters.join(", ")}`,
      );
      for (const action of finding.duplicate_actions) {
        const status = action.status ? ` status=${action.status}` : "";
        lines.push(
          `  - ${action.source} chapter=${action.chapter} action=${action.action}${status}`,
        );
      }
  ...
  lines.push(
    "",
    "## Policy",
    "- Dry-run only: this report does not modify NovelMemory.",
    "- Keep one earliest plant/register source per foreshadowing id.",
    "- Later touches must be logged as develop or reveal after checking the manuscript.",
  );
