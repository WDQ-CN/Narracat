...
let bundledDriver: SqliteDriver | null = null;

/** 懒加载默认驱动：core 模块被 Electron 主/工具进程 import 时不触碰 node-ABI 二进制（模块级 import 会直接 ERR_DLOPEN）。 */
function loadBundledDriver(): SqliteDriver {
  if (!bundledDriver) {
    const require = createRequire(import.meta.url);
    bundledDriver = require("better-sqlite3") as SqliteDriver;
  }
 ...
 */
export function openDatabase(dbPath: string, novelId: string, driver?: SqliteDriver): Database.Database {
  // 确保目录存在
  ...
  try {
    ...
    console.error(
      `[NovelMemory] sqlite-vec 加载失败，向量检索不可用: ${
