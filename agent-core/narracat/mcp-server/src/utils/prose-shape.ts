...
function excerpt(value: string, width = 40): string {
...
function splitParagraphs(text: string): Paragraph[] {
  ...
  for (const block of blocks) {
    ...
    if (!clean) continue;
    index += 1;
    if (clean.startsWith("#") || /^(?:[-+*]|\d+[.、])\s/.test(clean)) continue;
 ...
 */
function anaphoraSentences(paragraphs: Paragraph[]): Array<{ index: number; sentence: string }> {
  const hits: Array<{ index: number; sentence: string }> = [];
  for (const para of paragraphs) {
    if (para.hasQuote) continue;
    for (const sentence of para.sentences) {
      const clauses = sentence
        .split(/[，、；,;]/)
        .map((c) => c.trim())
        .filter((c) => hanziCount(c) >= 3);
      if (clauses.length < ANAPHORA_RUN) continue;
      let run = 1;
...
function countTerms(text: string, terms: string[]): Map<string, number> {
 ...
 */
export function scanProseShape(text: string): ProseShapeFinding[] {
  const paragraphs = splitParagraphs(text);
  if (paragraphs.length === 0) return [];

  const findings: ProseShapeFinding[] = [];
  ...
  // ⑤ 主干来得太晚
  const lateHits: Array<{ index: number; sample: string }> = [];
  for (const para of paragraphs) {
  ...
  // ⑥ 一句话四个「的」
  const heavyDe: Array<{ index: number; sentence: string }> = [];
  for (const para of paragraphs) {
    for (const sentence of para.sentences) {
  ...
  // ⑧ 段落开场重复
  const openerCounts = new Map<string, { count: number; first: number }>();
  for (const para of paragraphs) {
