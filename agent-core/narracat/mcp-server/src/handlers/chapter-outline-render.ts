...
import type { CharacterReference, ChapterOutlineItem } from "./validators.js";

/** payoff_beat 英文枚举 → 中文展示标签（渲染层；枚举 SSOT 在 outline-structure.json） */
export const PAYOFF_BEAT_LABEL: Record<string, string> = {
  face_slap: "打脸",
  level_up: "升级突破",
  windfall: "机缘横财",
  fame: "扬名",
...
export const END_HOOK_LABEL: Record<string, string> = {
  suspense: "悬念",
  danger: "危机",
  emotional: "情绪",
  none: "无",
...
export const PAYOFF_INTENSITY_LABEL: Record<string, string> = {
  small: "小",
  medium: "中",
  large: "大",
};
...
// ---------------------------------------------------------------------------
// 局部形态接口（只服务渲染，不导出、不改 validators.ts）
// ---------------------------------------------------------------------------

/** 旧形态字段集（scenes 列式） */
interface LegacyChapterOutlineFields {
  chapter: number;
  title: string;
  value_shift: string;
  ...
  foreshadowing_touch?: Array<{ id: string; action: string }>;
}

/** 新形态字段集（beats 骨架式） */
interface NewChapterOutlineFields {
  chapter: number;
  title: string;
  positioning: string;
  beats: string[];
...
function renderLegacy(
  ch: LegacyChapterOutlineFields,
  ctx: ChapterOutlineRenderContext,
): string {
  ...
  lines.push(
    `- 聚焦故事线: ${ch.storyline_focus
      .map((id) => {
        const name = storylineNames.get(id);
        return name ? `${id}「${name}」` : id;
      })
      .join("、")}`,
    `- 视角人物: ${ch.pov_character.name}`,
  );
  if (ch.ending_note) {
...
function renderNew(
  ...
  if (ch.must_deliver && ch.must_deliver.length > 0) {
    lines.push("", "## 必须落地", "");
    for (const m of ch.must_deliver) lines.push(`- ${m}`);
  }
  ...
  lines.push(
    `- 聚焦故事线: ${ch.storyline_focus
      ...
      .join("、")}`,
    `- 视角人物: ${ch.pov_character.name}`,
    `- 出场角色: ${ch.characters.map((c) => c.name).join("、")}`,
  );
  ...
  if (stateChanges.length > 0) {
    ...
    for (const sc of stateChanges) {
      ...
      lines.push(
        `- ${sc.character.name}: ${sc.dimension} ${opLabel}「${sc.value}」${sc.reason ? `（${sc.reason}）` : ""}`,
      );
    }
  }
  return lines.join("\n");
...
export function renderChapterOutlineMarkdown(
  ch: ChapterOutlineItem,
  ctx: ChapterOutlineRenderContext,
): string {
  // 旧形态（无 beats）走旧逻辑，向后兼容存量 ch-NNN.json
  if (!Array.isArray((ch as { beats?: unknown }).beats)) {
    return renderLegacy(ch as unknown as LegacyChapterOutlineFields, ctx);
  }
  return renderNew(ch as unknown as NewChapterOutlineFields, ctx);
}
