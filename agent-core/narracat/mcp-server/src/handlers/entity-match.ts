...
export function prepareNameIndex(aliasMap: Map<string, ResolvedCharacter>): NameEntry[] {
...
export function matchCharactersInText(
  ...
  const byCanonical = new Map<string, ResolvedCharacter>();
  for (const { name, resolved } of nameIndex) {
    ...
    for (;;) {
      ...
      if (free) {
        for (let i = idx; i < idx + name.length; i++) occupied[i] = true;
