...
export const FACT_VALID_AT_SQL =
  "COALESCE(event_chapter, from_chapter) <= ? AND (invalidated_at_chapter IS NULL OR invalidated_at_chapter > ?)";
 ...
 */
export const FACT_LATEST_ORDER_SQL =
  "COALESCE(event_chapter, from_chapter) DESC, CASE WHEN source = 'authored' THEN 1 ELSE 0 END DESC, from_chapter DESC, created_at DESC, rowid DESC";
...
export function isFactValidAt(row: TemporalFactRow, at: number): boolean {
 ...
 */
export function isSecretHiddenForChat(row: SecretGateRow, secretFilterOn: boolean): boolean {
 ...
 */
export function revalidateVictimsOf(
