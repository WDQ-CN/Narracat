...
const NARRATOR_ADDRESS_PHRASES: Record<string, string> = {
  first_person: "第一人称",
...
export function isNarratorAddress(value: string): value is NarratorAddress {
  return (NARRATOR_ADDRESS_VALUES as readonly string[]).includes(value);
}

/** 枚举值 → 中文短语（喂模型的渲染层）；未知值原样返回，不抛错。 */
export function narratorAddressPhrase(value: string): string {
