...
// ajv 8 CJS exports 与 NodeNext ESM 类型签名不兼容；运行时转译会处理 default interop
...
function loadJsonSchema(name: string): Record<string, unknown> {
  return JSON.parse(
    readFileSync(resolve(SCHEMAS_DIR, name), "utf-8"),
...
function buildChapterBatchSchema(): Record<string, unknown> {
  const defs = outlineSchema["$defs"];
  if (!defs || typeof defs !== "object") {
    throw new Error(
      "[validators] outline-structure.json 缺少 $defs/chapter_outline_batch——schema 结构变更需同步更新本文件",
    );
  }
  return {
...
function extractForeshadowingItemFragment(): Record<string, unknown> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const root = loadJsonSchema("foreshadowing-system.json") as any;
  const item = root?.properties?.registry?.items;
  if (!item || typeof item !== "object") {
    throw new Error(
      "[validators] 无法从 foreshadowing-system.json 提取 registry item fragment——schema 结构变更需同步更新本文件",
    );
  }
  return {
    $schema: "https://json-schema.org/draft/2020-12/schema",
...
const outlineValidator: AjvValidator = ajv.compile(outlineSchema);
// 书级段校验：同一 schema、required 去掉 volumes（两段制阶段一 a；$defs 随对象展开共享）
const outlineBookSchema = {
  ...outlineSchema,
  required: (outlineSchema["required"] as string[]).filter((key) => key !== "volumes"),
};
const outlineBookValidator: AjvValidator = ajv.compile(outlineBookSchema);
const chapterBatchValidator: AjvValidator = ajv.compile(buildChapterBatchSchema());
const extractionValidator: AjvValidator = ajv.compile(
  loadJsonSchema("memory-extraction.json"),
);
const reviewValidator: AjvValidator = ajv.compile(
  loadJsonSchema("review-report.json"),
);
const cascadeValidator: AjvValidator = ajv.compile(
  loadJsonSchema("cascade-impact-report.json"),
);
const foreshadowingItemValidator: AjvValidator = ajv.compile(
  extractForeshadowingItemFragment(),
);
const commitChapterValidator: AjvValidator = ajv.compile(COMMIT_CHAPTER_SCHEMA);
const consolidateValidator: AjvValidator = ajv.compile(CONSOLIDATE_SCHEMA);
const premiseValidator: AjvValidator = ajv.compile(premiseSchema);
const dialogueSamplesValidator: AjvValidator = ajv.compile(dialogueSamplesSchema);
const stateVocabularyValidator: AjvValidator = ajv.compile(stateVocabularySchema);
const characterEntityValidator: AjvValidator = ajv.compile(characterEntitySchema);
const authoredStateValidator: AjvValidator = ajv.compile(authoredStateSchema);
...
function fail(errors: ToolErrorItem[]): ValidationFail {
...
function fieldFromInstancePath(instancePath: string): string {
  if (!instancePath) return "(root)";
  return instancePath.replace(/^\//, "").replace(/\//g, ".");
}

function extractByPath(rootData: unknown, instancePath: string): unknown {
  if (!instancePath) return rootData;
...
function describeActual(value: unknown): string {
  ...
  if (rendered.length > 120) rendered = `${rendered.slice(0, 120)}…`;
  return `${t}: ${rendered}`;
}
 ...
 */
export function ajvErrorsToItems(
  errors: AjvErrorObject[],
  rootData: unknown,
  schemaName: string,
): ToolErrorItem[] {
  const items: ToolErrorItem[] = [];
  for (const err of errors) {
    const field = fieldFromInstancePath(err.instancePath);
    const actualValue =
      err.data !== undefined ? err.data : extractByPath(rootData, err.instancePath);
    ...
    if (err.keyword === "dependentRequired") {
      ...
      const present = (err.params.property as string) ?? "";
      const missing = (err.params.missingProperty as string) ?? "";
      const target = field === "(root)" ? missing : `${field}.${missing}`;
      items.push({
        field: target,
    ...
    if (err.keyword === "minLength" || err.keyword === "maxLength") {
      const len =
        typeof actualValue === "string" ? String(actualValue.length) : "非字符串";
      items.push({
        field,
...
export function validateOutlinePayload(value: unknown): ValidationResult {
  return runAjv(outlineValidator, value, "outline-structure.json");
}

export function validateOutlineBookPayload(value: unknown): ValidationResult {
  return runAjv(outlineBookValidator, value, "outline-structure.json（书级段，volumes 非必填）");
}

export function validateChapterOutlineBatch(value: unknown): ValidationResult {
  return runAjv(
    chapterBatchValidator,
    value,
    "outline-structure.json#/$defs/chapter_outline_batch",
  );
}
...
export function validateReview(value: unknown): ValidationResult {
  return runAjv(reviewValidator, value, "review-report.json");
}

export function validateForeshadowingItem(value: unknown): ValidationResult {
  return runAjv(
    foreshadowingItemValidator,
    value,
    "foreshadowing-system.json#registry.items",
  );
}

export function validateCommitChapter(value: unknown): ValidationResult {
  return runAjv(commitChapterValidator, value, "novel_commit_chapter 参数");
}

export function validateConsolidate(value: unknown): ValidationResult {
  return runAjv(consolidateValidator, value, "novel_consolidate 参数");
}

export function validatePremise(value: unknown): ValidationResult {
...
export function validateStateVocabulary(value: unknown): ValidationResult {
...
export function validateCharacterEntity(value: unknown): ValidationResult {
  return runAjv(characterEntityValidator, value, "character-entity.json");
}

/** 作者对角色结构化状态的直接修订 */
export function validateAuthoredState(value: unknown): ValidationResult {
 ...
 */
export function validateCascadeImpactReport(value: unknown): ValidationResult {
  const base = runAjv(cascadeValidator, value, "cascade-impact-report.json");
  const errors: ToolErrorItem[] = base.valid ? [] : [...base.errors];

  if (value && typeof value === "object" && !Array.isArray(value)) {
    const obj = value as Record<string, unknown>;
    const changeKind =
      obj.change_kind === undefined ? "chapter_rewrite" : obj.change_kind;
    ...
    if (changeKind === "chapter_rewrite") {
      if (
        typeof obj.rewritten_chapter !== "number" ||
        ...
        for (let i = 0; i < obj.affected_chapters.length; i++) {
          ...
          if (item && typeof item === "object" && !Array.isArray(item)) {
            const ch = (item as Record<string, unknown>).chapter;
...
// ============================================================
// 结构预算（novel_get_structure_budget 与 submit_outline 校验共用）
// ============================================================
...
export interface StructureBudget {
  ...
  volumes: { recommended: number; min: number; max: number };
  /** arc 跨度（章），按 tier 伸缩 */
  arc_span: { min: number; max: number };
  ...
  foreshadowing: { major: number; medium: number; small: number };
...
const ARC_SPAN_BY_TIER: Record<StructureTier, { min: number; max: number }> = {
  S: { min: 5, max: 15 },
  M: { min: 10, max: 25 },
  L: { min: 15, max: 35 },
  XL: { min: 20, max: 40 },
...
// ============================================================
// 书级大纲：结构语义 + 预算下限 + 里程碑核验（ajv 通过后调用）
// ============================================================
 ...
 */
export function resolveRevealChapter(
  targetReveal: string,
  volumeLastChapter: Map<number, number>,
): number | null {
 ...
 */
export function checkOutlineSemantics(payload: OutlinePayload): OutlineCheckResult {
  const errors: ToolErrorItem[] = [];
  const warnings: string[] = [];

  const volumes = [...payload.volumes].sort((a, b) => a.volume_no - b.volume_no);
  const volumeLastChapter = new Map<number, number>();
    ...
    for (const arc of arcs) {
      ...
      if (isFirstArc) {
        if (arc.chapter_start !== 1) {
          warnings.push(
            `首个 arc 从第 ${arc.chapter_start} 章开始（非第 1 章）——增量提交后续卷时正常，全新规划时请检查`,
          );
        }
        isFirstArc = false;
      ...
      volumeLastChapter.set(
        vol.volume_no,
        Math.max(volumeLastChapter.get(vol.volume_no) ?? 0, arc.chapter_end),
      );
    }
  }
  ...
  if (
    payload.storylines.length >= 3 &&
    !payload.storylines.some((sl) => sl.is_through_line === true)
  ) {
    warnings.push(
      "未标记任何全书贯穿故事线（is_through_line=true）——建议把主线/宿敌线标记为贯穿线，使其进 WCP 常驻层、远卷写作时不被卷滚动丢弃",
    );
  }
 ...
 */
export function checkOutlineBudget(
  ...
  for (const arc of arcs) {
    ...
    if (span > budget.arc_span.max) {
      ...
      if (arc.chapter_end === submittedChapters) {
        warnings.push(
          `末尾 arc ${arc.arc_id} 仅 ${span} 章（低于 ${budget.tier} 档下限 ${budget.arc_span.min}），收尾 arc 可接受`,
        );
      } else {
        errors.push({
          field: `arc ${arc.arc_id}`,
  ...
  if (payload.storylines.length < 2) {
    ...
    warnings.push(
      `故事线 ${payload.storylines.length} 条与预算 ${budget.storyline_budget} 条偏差较大`,
    );
  }

  const majorCount = payload.foreshadowing_registry.filter(
    (f) => f.type === "major",
  ).length;
  if (majorCount < 1) {
    errors.push({
  ...
  if (
    submittedChapters >= budget.total_chapters &&
 ...
 */
export function checkDilemmaMilestones(
  payload: OutlinePayload,
  totalChapters: number | null,
): OutlineCheckResult {
  if (payload.volumes.length === 0) return { errors: [], warnings: [] };
  ...
  let prevRank = 0;
  for (const vol of volumes) {
    if (!vol.dilemma_milestone) continue;
 ...
 */
export function checkForeshadowingPayoffTiming(payload: OutlinePayload): OutlineCheckResult {
  if (payload.volumes.length === 0) return { errors: [], warnings: [] };
  ...
  const chapterToArc = new Map<number, string>();
  ...
  const majorPayoffsByVolume = new Map<number, number>();
  for (const vol of volumes) majorPayoffsByVolume.set(vol.volume_no, 0);

  for (const fs of payload.foreshadowing_registry) {
    const revealChapter = resolveRevealChapter(fs.target_reveal, volumeLastChapter);
    if (revealChapter === null) continue; // 无法机械解析的锚点跳过，不臆测

    if (fs.type === "major") {
      const revealVolume = chapterToVolume.get(revealChapter);
      if (revealVolume !== undefined) {
        majorPayoffsByVolume.set(
          revealVolume,
          (majorPayoffsByVolume.get(revealVolume) ?? 0) + 1,
        );
        ...
        if (
          ...
          warnings.push(
            `伏笔 ${fs.id}（major）兑现跨度过长：埋设于第 ${plantedVolume} 卷（第 ${fs.planted_chapter} 章）拖到第 ${revealVolume} 卷（第 ${revealChapter} 章）兑现，跨 ${spanVolumes} 卷 / ${spanChapters} 章——major 可跨卷但不宜久悬不付利息，考虑中途加阶段性揭示或拉近兑现位`,
          );
        }
      }
      continue;
    ...
    if (fs.type === "small") {
      if (plantedArc !== undefined && revealArc !== undefined && plantedArc !== revealArc) {
        warnings.push(
          `伏笔 ${fs.id}（small）应在 arc 内兑现：埋设于 ${plantedArc}（第 ${fs.planted_chapter} 章）却拖到 ${revealArc}（第 ${revealChapter} 章）兑现——small 伏笔跨 arc 即延迟兑现，改为 medium 或拉近兑现位`,
        );
      }
      ...
      if (
        ...
        warnings.push(
          `伏笔 ${fs.id}（medium）应在本卷内兑现：埋设于第 ${plantedVolume} 卷（第 ${fs.planted_chapter} 章）却拖到第 ${revealVolume} 卷（第 ${revealChapter} 章）兑现——medium 伏笔跨卷即延迟兑现，改为 major 或拉近兑现位`,
        );
      }
    }
  }
  ...
  for (const vol of volumes) {
    ...
    if (count < minPerVolume) {
      warnings.push(
        `第 ${vol.volume_no} 卷 major 伏笔兑现仅 ${count} 条（建议每卷至少兑现 ${minPerVolume} 条）——长线悬念扎堆末卷兑现会让中段卷缺乏大爆发，把部分 major 的 target_reveal 提前到本卷`,
      );
    }
  }
 ...
 */
export function checkStructureRhythm(
  payload: OutlinePayload,
  budget: StructureBudget,
): OutlineCheckResult {
  const errors: ToolErrorItem[] = [];
  ...
  if (arcs.length >= T.arc_rhythm_min_arcs) {
    ...
    for (const len of lengths) freq.set(len, (freq.get(len) ?? 0) + 1);
    ...
    if (share > T.arc_mode_share_max) {
      errors.push({
        ...
        actual: `${arcs.length} 个 arc 中 ${modeCount} 个都是 ${modeLen} 章（占 ${Math.round(
          share * 100,
        )}%）`,
  ...
  if (
    sls.length >= T.storyline_min_for_stagger &&
  ...
  if (sls.length >= T.storyline_min_for_midpoint && budget.total_chapters > 0) {
    ...
    const hasMidpoint = sls.some(
      (s) =>
        s.planned_payoff_chapter !== undefined &&
        s.planned_payoff_chapter >= lo &&
        s.planned_payoff_chapter <= hi,
    );
    if (!hasMidpoint) {
      warnings.push(
        `${sls.length} 条故事线无一条 planned_payoff_chapter 落在全书中段区间（第 ${Math.round(
          lo,
        )}-${Math.round(
          hi,
        )} 章）——storyline-weave 包：中段收掉一些线再开新线，别把所有线都拖到结尾或不给收线章`,
      );
    }
  }
  ...
  for (const fs of payload.foreshadowing_registry) {
    ...
    if (fs.type === "small") {
      if (volAnchor) {
        warnings.push(
          `伏笔 ${fs.id}（small）用卷级粗锚点 ${fs.target_reveal} 标兑现——foreshadow-distance 包：small 伏笔应在 arc 内快速兑现并给确切章号，改用近距章号或降档为 medium/major`,
        );
        continue;
      }
      const reveal = Number(fs.target_reveal);
      if (!Number.isInteger(reveal) || reveal <= 0) continue;
      ...
      if (span > errMax) {
        ...
        warnings.push(
          `伏笔 ${fs.id}（small）跨 ${span} 章（埋第 ${fs.planted_chapter}、兑第 ${reveal}）超出单个 arc 跨度（${budget.tier} 档 ≤ ${budget.arc_span.max} 章）——foreshadow-distance 包：small 应 arc 内兑现，降档为 medium 或拉近兑现位`,
        );
      }
      continue;
    }
    ...
    if (fs.type === "major" && volAnchor && fs.planted_chapter <= earlyPlant) {
      warnings.push(
        `伏笔 ${fs.id}（major）早埋于第 ${fs.planted_chapter} 章（书前 ${Math.round(
          T.major_early_plant_ratio * 100,
        )}%）却用卷级粗锚点 ${fs.target_reveal} 标兑现——早揭的 major 可用精确章号替代 vol-NN 粗锚（远期 major 才适合卷级粗锚），让兑现位更可控`,
      );
    }
  }
...
export interface ChapterOutlineItem {
  ...
  foreshadowing_touch?: Array<{ id: string; action: "plant" | "develop" | "reveal" }>;
  state_changes?: StateChangeItem[];
}

export interface ChapterBatchCheckInput {
  /** chapter → {arcId, volumeNo}，由 arc_meta 表展开 */
  chapterArcIndex: Map<number, { arcId: string; volumeNo: number }>;
...
function firstMachineToken(text: string): string | null {
 ...
 */
export function checkChapterProseHygiene(chapters: ChapterOutlineItem[]): ToolErrorItem[] {
  const errors: ToolErrorItem[] = [];
  for (const ch of chapters) {
    const proseFields: Array<[string, string]> = [];
    if (typeof ch.positioning === "string") proseFields.push(["positioning", ch.positioning]);
    (ch.beats ?? []).forEach((b, i) => {
      if (typeof b === "string") proseFields.push([`beats[${i + 1}]`, b]);
    });
    (ch.must_deliver ?? []).forEach((m, i) => {
      if (typeof m === "string") proseFields.push([`must_deliver[${i + 1}]`, m]);
    });
 ...
 */
export function scanManuscriptProseHygiene(text: string): {
  errors: ToolErrorItem[];
  ...
  for (const para of text.split(/\n+/)) {
    ...
    if (c > maxAntithesisInParagraph) maxAntithesisInParagraph = c;
  }
  ...
  if (
    (enoughForDensity && antithesisPerKilo > MANUSCRIPT_ANTITHESIS_PER_KILO) ||
    ...
    errors.push({
      ...
      actual: `全章 ${antithesisCount} 处${
        maxAntithesisInParagraph >= 2 ? `（有一段连排 ${maxAntithesisInParagraph} 处）` : `，约 ${antithesisPerKilo.toFixed(1)}/千字`
      }${sample ? `（例：${sample}）` : ""}`,
...
export function checkChapterBatch(
  chapters: ChapterOutlineItem[],
  refs: ChapterBatchCheckInput,
): ChapterBatchCheckResult {
  const errors: ToolErrorItem[] = [];
  const warnings: string[] = [];
  const arcsCovered = new Set<string>();
 ...
 */
export function checkPayoffIntensityConsistency(
  chapters: ChapterOutlineItem[],
): { warnings: string[] } {
  const warnings: string[] = [];
  ...
  if (missing.length > 0) {
    warnings.push(
      `第 ${missing.join("、")} 章有 payoff_beat 但未标 payoff_intensity——补一个强度档位（small/medium/large），让 payoff-cadence 包的强度金字塔知识有字段可落`,
    );
  }
  return { warnings };
}
 ...
 */
export function checkOpeningRetention(
  chapters: ChapterOutlineItem[],
): OutlineCheckResult {
  const errors: ToolErrorItem[] = [];
  const warnings: string[] = [];
  ...
  if (!present.has(1)) return { errors, warnings }; // 非开局集（不含第 1 章）→ no-op
  ...
  if (goldenComplete) {
    const openingBeats = sorted.filter(
      (c) => c.chapter <= T.golden_chapters && c.payoff_beat != null,
    ).length;
  ...
  if (maxGap > T.max_opening_payoff_gap_warn) {
    warnings.push(
      `开局段第 ${worstStart}-${worstEnd} 章连续 ${maxGap} 章无 payoff_beat（超 ${T.max_opening_payoff_gap_warn} 章死区）——开局是读者最易流失段，按本书反馈回路补几处小爽，别留长空窗`,
    );
  }
 ...
 */
export function checkHookCadence(
  merged: Array<Pick<ChapterOutlineItem, "chapter" | "end_hook">>,
  batchChapters: Set<number>,
): { warnings: string[] } {
  const warnings: string[] = [];
  ...
  if (missing.length > 0) {
    warnings.push(
      `本批第 ${missing.join("、")} 章未填 end_hook——每章显式声明章末钩类型（悬念/危机/情绪），确属刻意缓冲的章填 none，别留空`,
    );
  }
  ...
  for (const c of sorted) {
    if (c.end_hook === "none") {
      ...
      if (run >= T.consecutive_none_warn) {
        warnings.push(
          `第 ${runStart}-${c.chapter} 章连续 ${run} 章无章末钩——头部网文约七成章有钩，成片无钩是追读死区；给其中至少一章排一个钩（悬念/危机/情绪），或确认这段确是刻意缓冲`,
        );
 ...
 */
export function checkOpeningArcPayoff(payload: OutlinePayload): OutlineCheckResult {
  if (payload.volumes.length === 0) return { errors: [], warnings: [] };
 ...
 */
export function checkPremiseSemantics(payload: PremiseCardsPayload): ToolErrorItem[] {
  const errors: ToolErrorItem[] = [];
 ...
 */
export function checkNarratorAddress(
  ...
  if (address.certainty === "open") return []; // 有意留白
 ...
 */
export function validateDialogueSamples(value: unknown): ValidationResult {
  return runAjv(dialogueSamplesValidator, value, "dialogue-samples.json");
}

// ============================================================
// StateVocabulary 机械语义校验（ajv 结构校验通过后追加）
// ============================================================
 ...
 */
export function checkStateVocabularySemantics(payload: StateVocabularyPayload): ToolErrorItem[] {
  const errors: ToolErrorItem[] = [];
  ...
  const byPredicate = new Map<string, StateVocabularyDimensionPayload[]>();
  for (const dim of dims) {
...
export function checkChapterWordCount(
  actual: number,
  target: number | null | undefined,
): WordCountShortfall | null {
  if (!target || target <= 0) return null;
  const ratio = actual / target;
  if (ratio >= WORD_COUNT_WARN_RATIO) return null;
