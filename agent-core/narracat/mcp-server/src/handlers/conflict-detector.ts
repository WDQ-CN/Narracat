...
export interface Conflict {
  ...
  chapter?: number;
  detail: string;
  facts: Array<{ id: string; object: string; chapter: number; from_chapter: number }>;
}
...
function entityKey(f: ConflictFactRow): string {
  return f.subject_character_uid ?? f.subject;
}

/** event 轴章号（默认 = ingestion）；用于「同一时刻」分组 */
function chapterOf(f: ConflictFactRow): number {
  return f.event_chapter ?? f.from_chapter;
}

function pushTo<K>(m: Map<K, ConflictFactRow[]>, k: K, f: ConflictFactRow): void {
  const arr = m.get(k);
  if (arr) arr.push(f);
  else m.set(k, [f]);
}
...
function distinctValues(objects: string[]): string[] {
  const reps: string[] = [];
  for (const o of objects) {
    if (!reps.some((r) => nearDuplicate(r, o))) reps.push(o);
  }
  return reps;
}

/** 在「同实体/同对」的分组上，按章找「同章 ≥2 互斥值」的冲突 */
function chapterClashes(
  groups: Map<string, ConflictFactRow[]>,
  type: ConflictType,
  detail: (subject: string, predicate: string | undefined, chapter: number, n: number) => string,
  predicateOf: (f: ConflictFactRow) => string | undefined,
): Conflict[] {
  const out: Conflict[] = [];
  for (const group of groups.values()) {
    const byChapter = new Map<number, ConflictFactRow[]>();
    for (const f of group) pushTo(byChapter, chapterOf(f), f);
...
export function detectConflicts(
  facts: ConflictFactRow[],
  opts: { chapter?: number } = {},
): Conflict[] {
  const conflicts: Conflict[] = [];

  // ① 设定漂移：单值谓词，同实体 + 同章 出现 ≥2 互斥值（跨章演变豁免、同章近重复归并）
  const byEntityPred = new Map<string, ConflictFactRow[]>();
  for (const f of facts) {
    if (f.predicate === "relationship" || !SINGLE_VALUE_PREDICATES.has(f.predicate)) continue;
  ...
  conflicts.push(
    ...chapterClashes(
      byEntityPred,
      "state_divergence",
      (subject, predicate, chapter, n) =>
        `「${subject}」第${chapter}章的 ${predicate} 同章出现 ${n} 个互斥值，疑似设定漂移`,
      (f) => f.predicate,
    ),
  );

  // ② 死而复生：status 含终结词后，同实体更晚章仍有新 fact
  const byEntity = new Map<string, ConflictFactRow[]>();
  for (const f of facts) pushTo(byEntity, entityKey(f), f);
  for (const group of byEntity.values()) {
    const terminal = group.find(
      (f) => f.predicate === "status" && TERMINAL_STATUS_WORDS.some((w) => f.object.includes(w)),
    );
  ...
  const byPair = new Map<string, ConflictFactRow[]>();
  for (const f of facts) {
    if (f.predicate !== "relationship") continue;
  ...
  conflicts.push(
    ...chapterClashes(
      ...
      (subject, _predicate, chapter, n) =>
        `「${subject}」第${chapter}章同章出现 ${n} 个互斥关系值，疑似关系状态未收敛`,
      () => undefined,
    ),
  );
...
/** 机械渲染可读冲突报告 */
export function renderConflictReport(conflicts: Conflict[]): string {
  if (conflicts.length === 0) return "未检出时序 / 状态冲突。";
  const lines = [`检出 ${conflicts.length} 处潜在冲突（仅标注，需人工确认 / 修订）：`, ""];
  conflicts.forEach((c, i) => {
    lines.push(`${i + 1}. [${TYPE_LABEL[c.type]}] ${c.detail}`);
    for (const f of c.facts) lines.push(`   - 第${f.chapter}章：${f.object}`);
  });
  return lines.join("\n");
}

export async function novelDetectConflicts(
  args: Record<string, unknown>,
  ctx: ToolContext,
): Promise<unknown> {
  const rawChapter = args["chapter"];
  const chapter =
    typeof rawChapter === "number" && Number.isInteger(rawChapter) && rawChapter >= 1
      ? rawChapter
      : undefined;

  const facts = ctx.db
    .prepare(
      `SELECT id, subject, subject_character_uid, subject_character_b_uid, predicate, object, from_chapter, event_chapter
       FROM facts
       WHERE novel_id = ? AND invalidated_at_chapter IS NULL`,
    )
    .all(ctx.novelId) as ConflictFactRow[];
