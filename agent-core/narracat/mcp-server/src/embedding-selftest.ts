...
export interface EmbeddingSelfTestReport {
  ok: boolean;
  modelLoad: SelfTestStep & { modelName?: string; dim?: number };
  embed: SelfTestStep & { dim?: number; normalized?: boolean; durationMs?: number };
  sqliteVec: SelfTestStep;
  retrieval: SelfTestStep & { hit?: boolean; topDistance?: number };
}
...
export async function runEmbeddingSelfTest(
  options: EmbeddingSelfTestOptions = {},
): Promise<EmbeddingSelfTestReport> {
  const report: EmbeddingSelfTestReport = {
    ok: false,
    modelLoad: { ok: false },
    embed: { ok: false },
    sqliteVec: { ok: false },
  ...
  try {
    db = (options.createDatabase ? options.createDatabase() : new Database(":memory:")) as Database.Database;
    sqliteVec.load(db);
  ...
  if (db && report.sqliteVec.ok && queryEmbedding && report.embed.ok) {
    try {
      ...
      if (!inited) {
        ...
        if (results.length > 0) report.retrieval.topDistance = results[0].distance;
        if (!hit) report.retrieval.error = "KNN 未命中刚写入的向量";
      }
    } catch (error) {
...
function emit(report: EmbeddingSelfTestReport, exitCode: number): void {
