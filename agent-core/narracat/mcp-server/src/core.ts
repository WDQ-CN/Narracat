...
export async function createToolContext(options: CreateToolContextOptions = {}): Promise<ToolContext> {
...
const defaultLog: RunToolLogger = (line) => console.error(line);
...
export async function runTool(
  ...
  try {
    ...
    const validationFailed = typeof result === "object" && result !== null && (result as { ok?: unknown }).ok === false;
...
export function createLazyToolRunner(options: CreateLazyToolRunnerOptions): {
  runTool(name: string, args: Record<string, unknown>): Promise<RunToolResult>;
  getContext(): Promise<ToolContext>;
} {
  const log = options.log ?? defaultLog;
  let contextPromise: Promise<ToolContext> | null = null;
  function getContext(): Promise<ToolContext> {
    if (!contextPromise) {
...
export async function runStartupBackfills(ctx: ToolContext, log: RunToolLogger = defaultLog): Promise<void> {
  const steps: Array<[string, (ctx: ToolContext) => Promise<unknown>]> = [
