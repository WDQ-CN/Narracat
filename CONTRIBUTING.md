...
> 本仓另有面向内部协作的 `CLAUDE.md` / `AGENTS.md` / `agent-core/narracat/CLAUDE.md` 等文件——它们只服务日常开发协作，不随开源导出、公开仓里看不到。本文件已经是这条纪律面向外部贡献者的完整版本，不需要也无法查阅那些文件。
...
`agent-core/narracat/` 的其余工程纪律（写权限模型、schema 契约、CI lint 清单细节）由维护者在内部开发仓维护，不随开源导出——本文件已经收录你作为外部贡献者需要知道的全部内容，不需要额外查阅。
